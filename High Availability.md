...
- 한 장비/라우터/방화벽이 죽어도 서비스가 멈추지 않도록  
    **중복(redundancy)**를 가지는 구조 → **HA, 무정지 서비스**.

### 3-2. Firewall HA – L4/L7 스위치 기반

- 여러 대의 방화벽 앞에 **L4/L7 스위치**를 두고,
    - 해시 `H(src_ip XOR dst_ip) mod N` 같은 방식으로 세션을  
        여러 방화벽에 **균등 분산**.
- 단점:
    - L4/L7 스위치 장비가 비싸고 복잡.
    - 중간에 방화벽 하나가 죽으면 **그 방화벽으로 가던 세션은 모두 끊김**.

### 3-3. Router HA – VRRP (10/30 녹음)
...
- 해결:
    - VRRP 광고에 **인증(예: 공유 비밀키 기반, 해시)**을 붙여  
        “메시지가 위조되지 않았음”을 검증.

### 3-4. 데이터센터 이중화 & DR 센터 (11/06 녹음)
