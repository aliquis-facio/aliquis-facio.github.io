...
- **Loss function** 복습:
    ...
    - 데이터셋에 대한 손실: $$\text{Loss} = \frac{1}{n}\sum_{i=1}^n L(f(x_i), y_i)$$
    - 손실이 작을수록 좋은 분류기.
...
- 우리가 최소화하는 건 원래: $$\min_{w,b} \text{loss}(w,b)$$
    (훈련 데이터 기준)
- **정규화(Regularization)**:
    - 손실에 **추가 항(regularizer)을 더한 것**: $$\min_{w,b} \ \text{loss}(w,b) + \lambda ,\text{regularizer}(w,b)$$
...
- **Gradient descent 업데이트:** $$w_j = w_j + \eta \sum_i y_i x_{ij} \exp(-y_i(w^\top x_i + b)) - \eta \lambda w_j$$
...
- 업데이트: $$w_j = w_j +\eta \sum_i y_i x_{ij}\exp(-y_i(w^\top x_i + b)) - \eta\lambda ,\text{sign}(w_j)$$
- **효과:**
    - $w_j>0$: 일정 값만큼 감소
    - $w_j<0$: 일정 값만큼 증가  
