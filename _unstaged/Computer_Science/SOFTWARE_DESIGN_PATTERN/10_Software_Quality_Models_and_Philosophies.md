# 1. 소프트웨어 품질 모델과 철학
## 1.2. 품질이란?
1. 명세 적합성(Conformance to Specification): 측정 가능한 특성이 미리 정의된 명세를 만족하는 제품과 서비스의 문제로 품질을 정의하는 관점. 제품/서비스가 사전에 정의된 명세를 충족하는가
2. 고객 요구 충족(Meeting Customer Needs): 측정 가능한 특성과는 무관하게 품질을 정의하는 관점. 명세와 무관하게 고객의 기대를 만족시키는가
...
품질 관리 원칙
1. 제품과 서비스 개선의 일관된 목적을 수립하라.
2. 새로운 철학을 받아들이고 학습 조직으로 변화하라.
3. 대량 검사를 없애고 처음부터 품질을 내재화하라.
4. 가격 경쟁 대신 총비용 최소화를 추구하라.
5. 끊임없이 생산 및 서비스 시스템을 개선하라.
6. 교육과 훈련을 제도화하라.
7. 감독자는 명령이나 처벌이 아니라 리더십을 발휘하라.
8. 두려움을 몰아내라. 질문하지 못하는 환경은 품질 향상을 막는다.
9. 부서 간 장벽을 허물어 협력하게 하라.
10. 구호, 표어, 숫자 목표를 제거하라.
11. 할당량(quota)을 폐지하라. 숫자만을 목표로 하면 비효율이 생긴다.
12. 직무 자부심을 가로막는 장벽을 제거하라.
13. 교육과 학습을 장려하는 제도를 마련하라.
14. 품질 혁신을 실현할 행동 계획을 수립하라.
...
품질은 객관적 품질(명세 적합성)과 주관적 품질(고객 만족)을 모두 포괄한다.
...
1. 제품 수정(Product Revision): 변화에 대응할 수 있는 능력
...
2. 제품 전이(Product Transition): 새로운 환경에 적응할 수 있는 능력
...
3. 제품 운용(Product Operations): 실제 동작 특성
...
![20250902232644](/_image/2025-09-02-23-26-44.png)
...
![2025-09-02-23-33-29](/_image/2025-09-02-23-33-29.png)
...
McCall 모델처럼 계층적 구조, 유지보수성에 중점을 둠

상위 특성(High-level Characteristics): 사용자가 소프트웨어를 구매할 때 던지는 기본 질문 반영
- 즉시 활용성(As-is Utility): 지금 당장 신뢰성 있고 효율적으로 사용할 수 있는가?
- 유지보수성(Maintainability): 쉽게 이해·수정·재테스트할 수 있는가?
- 이식성(Portability): 환경이 바뀌어도 사용할 수 있는가?
...
![2025-09-02-23-37-32](/_image/2025-09-02-23-37-32.png)
...
![2025-09-02-23-38-16](/_image/2025-09-02-23-38-16.png)
...
다섯 가지 범주
- Functionality (기능성): 기능 집합, 보안, 성능 범위 등
- Usability (사용성): 인간공학, 미적 요소, UI 일관성, 온라인/상황별 도움말, 사용자 매뉴얼, 교육 자료 등
- Reliability (신뢰성): 오류 빈도와 심각도, 복구 가능성, 예측 가능성, 정확성, 평균 고장 간격(MTBF) 등
- Performance (성능): 속도, 효율성, 가용성, 처리량, 응답 시간, 복구 시간, 자원 사용량 등
- Supportability (지원성): 테스트 용이성, 확장성, 적응성, 유지보수성, 호환성, 설정 가능성, 설치 가능성, 지역화(국제화) 등

F(기능성): 기능적 요구사항, URPS(비기능적 요구사항): 비기능적 특성으로 분류

### 1.4.4. by R. Geoff Dromey
소프트웨어 품질 평가는 제품마다 다르다는 점을 강조 -> **보다 동적인 모델링 접근법**이 필요하다고 주장
소프트웨어 제품의 속성과 소프트웨어 품질 특성 간의 관계를 명확히 연결하려 함.

![2025-09-02-23-44-26](/_image/2025-09-02-23-44-26.png)
...
5단계 프로세스
1. 평가에 필요한 고수준 품질 속성을 선택한다.
2. 시스템의 컴포넌트/모듈을 나열한다.
3. 각 컴포넌트의 품질을 담보하는 속성을 식별한다.
4. 각 속성이 품질 속성에 어떻게 영향을 미치는지 결정한다.
5. 모델을 평가하고 약점을 확인한다.
...
### 1.4.5. ISO
#### 1.4.5.1. ISO 9000
![2025-09-02-23-45-33](/_image/2025-09-02-23-45-33.png)
...
**ISO 9001**은 모든 산업 분야 조직에 적용 가능한 국제 품질경영시스템(QMS) 표준
내부적으로는 조직의 프로세스와 방법론을, 외부적으로는 제공되는 제품과 서비스의 품질 보증·관리·통제 등을 다룬다.

ISO 9001은 프로세스 지향적 품질 관리 접근법을 취한다. 즉, 다음과 같은 프로세스를 설계, 문서화, 구현, 지원, 모니터링, 통제, 개선할 것을 요구한다:
- 품질 관리 프로세스
- 자원 관리 프로세스
- 규제 조사 프로세스
- 시장 조사 프로세스
...
![2025-09-03-00-01-58](/_image/2025-09-03-00-01-58.png)
Figure 11: Maturity Levels of (SW-)CMM

**SW-CMM**은 소프트웨어 품질을 프로세스 관점에서 다룸
다섯 단계의 성숙도 레벨:
1. 초기(Initial): 예측 불가, 비공식적, 영웅적 개인 의존
2. 반복 가능(Repeatable): 기본적인 프로젝트 관리 프로세스 적용
3. 정의됨(Defined): 조직 차원의 표준 프로세스 정의 및 활용
4. 관리됨(Managed): 정량적 품질 및 프로세스 관리
5. 최적화됨(Optimizing): 지속적 개선 및 혁신

![2025-09-03-00-02-46](/_image/2025-09-03-00-02-46.png)
Figure 12: The two representations of the CMMI model.

SW-CMM은 이후 CMMI로 확장. 소프트웨어뿐만 아니라 시스템 엔지니어링, 프로젝트 관리, 지원 영역까지 포함
CMMI의 성숙도 레벨 = SW-CMM의 Level + **Level 0(불완전)**

### 1.4.8. 식스 시그마(Six Sigma)
본질은 **고객 중심 측정과 목표 설정**을 통해 수익성을 향상하는 경영 철학
- 고객의 목소리(Voice of the Customer, VOC)를 경청하고
- 고객 요구사항을 정량적이고 측정 가능한 지표로 전환하며
- 결함을 백만 개당 3.4개 이하로 줄이는 것을 목표로 한다.

식스 시그마는 품질을 단순한 규격 준수나 만족도 개념을 넘어, **비즈니스 성과와 직결된 관리 체계**로 확장한 접근

## 1.5. 결론 및 논의
품질은 다차원적이고 역동적인 개념: 명세 준수와 고객 만족을 동시에 고려해야 함.
