...
SFTP는 이름에 FTP가 들어가지만 FTP의 보안 확장판이 아니다.

## 2. FTP

FTP(File Transfer Protocol)는 클라이언트와 서버 사이에서 파일을 업로드·다운로드하기 위한 응용 계층 프로토콜이다.
...
일반적으로 제어 연결에는 TCP 21번 포트를 사용한다. 능동 모드에서는 서버 데이터 연결에 TCP 20번 포트가 사용될 수 있고, 수동 모드에서는 서버가 알려준 별도의 포트를 사용한다.
...
현재는 NAT·공유기·방화벽과의 호환성 때문에 **수동 모드가 일반적**이다.

### 보안상의 한계

일반 FTP에는 자체 암호화 기능이 없다.
...
따라서 공개 인터넷에서 평문 FTP를 새로 구축하는 것은 일반적으로 권장되지 않는다.

## 3. FTPS

FTPS는 기존 FTP 통신에 TLS를 적용한 방식이다.

```text
FTP + TLS/SSL = FTPS
```

FTP의 명령과 연결 구조를 유지하기 때문에 기존 FTP 시스템과 호환하기 쉽다.
...
FTPS는 새로운 시스템보다는 **기존 FTP 기반 업무 시스템에 보안을 추가할 때** 적합하다.

## 4. SFTP

SFTP(SSH File Transfer Protocol)는 SSH 연결 위에서 파일을 전송하는 프로토콜이다.
...
일반적으로 TCP 22번 포트 하나를 사용하고, 명령과 파일 데이터가 같은 암호화 연결을 통해 전달된다.
...
신규 Linux 서버나 자동화된 보안 파일 전송에서는 일반적으로 **SFTP가 우선 선택지**이다.

## 5. vsftpd

vsftpd는 `Very Secure FTP Daemon`의 약자로, Linux·Unix 서버에서 FTP 서비스를 제공하는 오픈소스 서버 프로그램이다.
...
여기서 `ssl_enable=YES`와 인증서 관련 설정을 적용하면 vsftpd를 FTPS 서버로 운영할 수 있다.

### vsftpd가 하지 않는 것

vsftpd는 SFTP 서버가 아니다.
...
SFTP 서비스는 일반적으로 OpenSSH의 `sshd`와 `sftp-server` 또는 내부 SFTP 서브시스템이 제공한다.
...
FTP는 ARPANET에 연결된 서로 다른 컴퓨터 사이에서 파일을 효율적으로 전달하기 위해 만들어졌다. 당시에는 운영체제와 파일 표현 방식이 달랐기 때문에 파일 구조와 문자 표현 방식까지 고려해야 했다.

### RFC 959 이후

1985년 발표된 RFC 959에서 FTP 명령과 연결 구조가 정리됐다. 오늘날 사용되는 FTP도 이 구조를 기반으로 한다.

하지만 FTP는 신뢰할 수 있는 초기 네트워크를 전제로 설계됐기 때문에 암호화와 현대적인 인증 기능이 부족했다.

### 보안 전송 방식의 등장

인터넷이 확대되면서 계정과 파일을 평문으로 전송하는 문제가 부각됐다.

* FTPS는 기존 FTP에 TLS를 추가
* SFTP는 SSH 기반의 별도 프로토콜로 발전

OpenSSH에는 2000년부터 SFTP 서버 기능이 포함되기 시작했고, Linux 서버 환경을 중심으로 SFTP가 널리 보급됐다.

### HTTPS와 클라우드로 전환

공개 파일 다운로드는 FTP보다 HTTPS와 CDN이 적합해졌다. 애플리케이션에서는 REST API, 오브젝트 스토리지, 사전 서명 URL 방식이 보편화됐다.

FTP가 하나의 범용 파일 전송 수단이던 시대에서, 현재는 목적에 따라 여러 기술을 선택하는 구조로 바뀐 것이다.

## 8. 현재(2026년 9월) 사용 경향

정확한 전 세계 프로토콜별 점유율을 보여주는 통일된 공신력 있는 통계는 제한적이다. 다만 주요 서버·클라우드 제품의 지원 현황과 보안 특성을 기준으로 보면 다음과 같이 정리할 수 있다.
...
신규 시스템을 구축한다면 일반적인 우선순위는 다음과 같다.
...
이 경우에도 계정 제한, IP 접근 제어, 전용 VLAN, 최소 권한, 로그 모니터링 등의 보완책이 필요하다.

## 10. 최종 결론

FTP는 파일 전송 기술의 기반을 만든 중요한 프로토콜이지만, 암호화되지 않는다는 근본적인 한계가 있다.

현재는 용도에 따라 다음과 같이 구분하는 것이 적절하다.
