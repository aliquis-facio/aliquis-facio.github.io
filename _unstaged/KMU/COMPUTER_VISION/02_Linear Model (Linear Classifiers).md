# Linear Model (Linear Classifiers)
## 1. 문제 세팅: 이미지 분류 & CIFAR-10

![|500x370](2025-11-24-18-41-47.png)
...
- 이미지: $x \in \mathbb{R}^{3072}$, 3072개의 픽셀 개수, 3072 차원
- 파라미터(가중치): $W, b$
...
![|500x240](2025-11-24-18-45-12.png)
...
- 입력 벡터 $x$에 상수 $c$를 곱하면, 출력도 그대로 $c$배가 된다
- 즉, 입력 이미지 밝기를 반으로 줄이면(모든 픽셀 값에 $\frac{1}{2}$를 곱한다) **점수들도 정확히 절반으로 줄어든다**.
- 이런 예측 가능성이 선형 모델의 장점이지만
	- 출력이 **비율대로** 변하니까, 모델의 동작이 **예측 가능하고 해석하기 쉬움**
- 동시에 표현력이 제한된다는 것을 의미.
	- 어떤 물체는 **밝기가 조금 달라져도 같은 물체로 인식**해야 함 → 입력이 0.8배, 1.2배 바뀌어도 **출력은 거의 같아야** 하는데 선형 모델은 무조건 0.8배, 1.2배로 같이 바뀜.
	- 조명, 대비, 노이즈, 비선형적인 효과(포화 등)는 입력과 출력 사이가 **단순 비례 관계가 아님**.
	- 복잡한 결정 경계(휘어진 곡선, 꼬여 있는 모양)를 만들기 어렵고  주로 “직선/평면”으로만 나누게 됨.
...
![|500x280](2025-11-24-18-52-21.png)
