...
5. **다양한 인터프리터에서 동일한 패턴 반복**
    - SQL, NoSQL, OS Command, ORM, LDAP, XPath, EL/OGNL, 템플릿 엔진(SSTI) 등 **“입력을 해석해 실행하는” 모든 인터프리터에서 동일한 개념으로 반복**됨.
6. **테스트 부족 또는 편향된 테스트**
    - 소스 코드 리뷰 없이, 또는 **자동화 테스트(SAST/DAST/IAST, Fuzzing) 없이** 일부 경로만 수동 테스트하는 경우 → 파라미터, 헤더, URL, 쿠키, JSON, SOAP, XML 입력 경로 중 일부만 점검되어 인젝션 취약점이 남는 경우가 많음.
7. **LLM·프롬프트 인젝션과의 연관**
    - 최근에는 LLM(대규모 언어 모델)을 대상으로 하는 **프롬프트 인젝션**이 별도 문서(OWASP LLM Top 10)에서 다뤄지지만, “입력이 모델의 지시/명령 구조에 섞여 들어가 원래 의도와 다른 행동을 유도한다”는 점에서 **Injection의 일반화된 형태**로 볼 수 있음.

요약하면, **“데이터와 명령/구조가 섞여 있는 곳”은 모두 Injection 후보 지점**이며, 입력이 해석되는 곳마다 동일한 패턴의 위험이 존재합니다.
...
1. **가장 우선: 안전한 API / 파라미터화된 인터페이스 사용**
    - 인터프리터를 직접 호출하지 않고, **파라미터 바인딩을 제공하는 안전한 API** 또는 안전하게 구현된 **ORM** 을 사용합니다.
    - 저장 프로시저(Stored Procedure)를 사용하더라도, 내부에서 `EXECUTE IMMEDIATE`, `exec()` 등으로 **문자열을 다시 붙여 실행하면 여전히 인젝션 가능**하다는 점을 기억해야 합니다.
2. **서버 측 화이트리스트 기반 입력 검증 (Positive Validation)**
    - 가능한 경우, 입력값에 대해 **허용 가능한 패턴(화이트리스트)을 기준**으로 검증합니다.
    - 다만, 텍스트 영역이나 모바일 API처럼 특수 문자를 허용해야 하는 경우가 많아 **이것만으로 완전한 방어가 되지는 않으므로, 다른 방어와 반드시 병행해야 합니다.**
3. **불가피한 동적 쿼리에는 컨텍스트별 이스케이프 적용**
    - 부득이하게 동적 쿼리/명령을 사용해야 한다면, 각 인터프리터(SQL, 쉘, LDAP, XPath 등)에 맞는 **컨텍스트 인지(Escape) 함수를 사용**하여 특수 문자를 적절히 이스케이프합니다.
    - 단, **테이블명·컬럼명 등 구조 자체를 사용자 입력으로 받는 것은 매우 위험**하며, 이런 구조 요소는 이스케이프만으로 안전을 보장할 수 없습니다.
4. **SAST/DAST/IAST + Fuzzing을 CI/CD에 통합**
    - 소스 코드 단계에서는 **SAST(정적 분석)** 로 인젝션 패턴을 탐지
    - 런타임에서는 **DAST(동적 분석), IAST(인터액티브 분석), Fuzzing** 을 통해 파라미터·헤더·쿠키·JSON·XML 등 모든 입력 경로를 자동으로 때려 보면서 취약점을 찾습니다.
    - 이 도구들을 **CI/CD 파이프라인에 통합**해, 운영 배포 전에 인젝션을 최대한 걸러냅니다.
5. **코딩 가이드·프레임워크 레벨 정책 수립**
    - 개발 표준으로, “문자열로 쿼리 조합 금지”, “ORM 쿼리 빌더/파라미터 API 의무 사용”, “템플릿 엔진의 안전 모드·자동 이스케이프 기능 기본 활성화” 등을 정해 **프레임워크 레벨에서 안전 패턴을 강제**합니다.
6. **에러 메시지·로그를 통한 구조 노출 방지**
    - 인젝션 시도에 의해 발생한 에러가 **쿼리 문자열, 템플릿 코드, 내부 경로** 등을 그대로 노출하지 않도록 에러 처리와 로깅 방식을 설계합니다(A02, A09와도 연결되는 부분).
7. **테스트 케이스에 인젝션 시나리오 포함**
    - 기능 테스트(정상 케이스)만이 아니라, **SQL/OS/템플릿/XSS/LDAP/XPath 인젝션 페이로드**를 테스트 케이스로 포함시켜 개발·QA 단계에서 반복적으로 검증합니다.
...
여기서 `id` 파라미터는 사용자가 브라우저 URL에 지정하는 값이며, 애플리케이션은 이를 **아무 검증 없이 문자열로 이어 붙여 쿼리를 생성**합니다.
...
- 여기서 `--` 이후는 주석으로 처리되어 나머지 구문은 무시되며, 쿼리의 의미는 **원래 의도와 전혀 다른 형태**로 바뀝니다.

단순한 예에서는 SLEEP(10)으로 **타임 딜레이(Blind SQLi 테스트)** 를 유발하거나, 조금 더 공격적인 경우에는 `UNION SELECT` 를 이용해 다른 테이블의 민감 데이터(계정 목록, 비밀번호 해시 등)를 가져오거나, 갱신·삭제·스토어드 프로시저 호출 등을 수행해 **데이터 파괴나 권한 상승**으로 이어질 수 있습니다.
...
위 코드처럼 **문자열 안에 HQL 구조와 사용자 입력을 직접 섞어 넣으면**, 사실상 앞선 SQL 예시와 완전히 동일한 인젝션 취약점이 발생합니다.

공격자는 동일하게 `id=' UNION SELECT SLEEP(10);--` 와 같은 값을 집어넣어 쿼리의 의미를 바꾸고, 타임 기반 블라인드 인젝션을 수행할 수 있습니다.
더 나아가, HQL 문법을 악용해 특정 레코드를 임의로 조회·수정하거나, 연관 엔티티를 따라가며 **예상치 못한 범위의 데이터에 접근**할 수 있습니다.

이 시나리오는 ORM/프레임워크를 사용하더라도, **“문자열로 쿼리를 만드는 패턴” 자체를 사용하면 인젝션에서 자유롭지 못하다**는 것을 보여 줍니다.
