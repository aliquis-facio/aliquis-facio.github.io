...
- **위협(Threat)**
    - 과거 공격의 근원이었거나, 앞으로 공격의 근원이 될 수 있는 **사람 / 그룹 / 조직 / 외부 압력**.
    - 예: 해커, 내부자, 범죄자, 경쟁 정보 전문가, 테러리스트, 정보 전사 등.

### 2) 소프트웨어 보안(Software Security)과 CERT/CC
...
> ⇒ 정수는 “그냥 쓰는 타입”이 아니라,  
> **범위/부호/형변환 규칙**까지 이해하고 써야 보안 문제를 피할 수 있음.
...
## 6. 수업에서 강조한 포인트 (실무/시험 관점)

녹음 내용 기준, 교수님이 secure coding과 관련해 반복적으로 강조한 흐름은 다음과 비슷함
...
마지막으로, 암기용으로 **용어/개념만 압축**해 두면:
- Threats: 해커, 내부자, 범죄자, 경쟁 정보 전문가, 테러리스트, 정보 전사
- Software Security: 개발적(시큐어 디자인+구현) + 운용적(안전한 시스템/네트워크)
- Security Policy: 민감 자원 보호를 위한 규칙·관행
- Software Defect / Security Flaw / Vulnerability / Exploit / Mitigation
- C/C++ 특징: 낮은 추상화, 타입/경계 보호 부족, 레거시 코드 위험
- Integers: 표현 범위, wrap-around, 정수 변환, 프로모션, 오버플로·언더플로
- CERT 규칙 예: INT30/31/32/35, STR35, MSC34, EXP33/34
- Strings: C-style vs C++ string, 널 종료, truncation, buffer overflow, 안전 함수(_s 계열)
- Dynamic Memory: `malloc/calloc/realloc/free`, new/delete, uninitialized, NULL 체크, use-after-free, double free, malloc(0)