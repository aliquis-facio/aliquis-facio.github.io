...
- 목표: 주어진 목적 함수  
    ...
    의 해를 **해석적으로(analytical)** 구하기 어려울 때, **수치적으로** 최소점을 찾는 방법인 **경사하강법(Gradient Descent)** 를 설명.
...
- 알고리즘은 **연속 두 스텝의 함수값 차이**가 충분히 작아지면 종료:  
    [  
...
- θ 업데이트:  
    ...
    \frac{1}{n}\sum_{i=1}^{n}  
    (\theta^{(t-1)T} x^{(i)} + \theta_0^{(t-1)} - y^{(i)})x^{(i)}
...
필요하면, 이 내용을 **수식 중심 요약** / **그림·직관 중심 요약** / **시험 대비 핵심 포인트 정리(문제 포맷)** 버전으로도 다시 정리해줄게요.