# 6. Backpropagation with Computational Graph
...
- 역할:
    ...
    - **“gradient router”**  
        → 업스트림 그레디언트를 **max를 낸 쪽으로만** 보내고, 나머지는 0.
...
- Softmax는 **raw score(logit)** 를 **확률**로 바꾸는 함수:  
    [  
    p_k = \frac{e^{s_k}}{\sum_j e^{s_j}}  
    ]
    
- 다중 클래스 분류에서 **크로스 엔트로피 손실**:  
    [  
    L_i = -\log p_{y_i}  
    ]  
...
- 슬라이드는 자세한 유도 대신, **softmax의 gradient** 에 대한 참고 링크로 넘기고,
    ...
    - 실제로는  
        [  
        \frac{\partial L}{\partial s_k} = p_k - 1[k = y]  
        ]  
...
3. **Backward pass**
    ...
    - 그래프의 **역순**으로 순회하면서:
        ...
        - 파라미터에 대한 그레디언트를 누적.
4. 계산된 (\frac{\partial L}{\partial W}) 로 **경사하강법(gradient descent)** 업데이트 수행:  
    [  
