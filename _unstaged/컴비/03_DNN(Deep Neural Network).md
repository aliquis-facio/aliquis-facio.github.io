...
수식으로 쓰면:
$$h = \sigma(W_1 x) \quad (\text{히든 레이어})$$
$$s = W_2 h \quad (\text{클래스 점수})$$
...
-  활성화 함수: **ReLU (Rectified Linear Unit)**: $$\text{ReLU}(z) = \max(0, z)$$
- 특징
    - 입력이 음수 → 0
    - 입력이 양수 → 그대로 통과
...
- 하지만 우리가 가지고 있는 건 보통 **트레이닝 데이터**뿐.
    - 그래서 우리는 “트레인 성능을 올리되, 테스트 성능이 떨어지지 않게”  
        모델과 학습 전략을 설계해야 함.
...
- **실제 연구/서비스에서는**:
    - 데이터 수집 & 정제(레이블링 포함)가 가장 비싼 작업이며  
        딥러닝 성능의 진짜 bottleneck이 되기도 한다고 언급.
