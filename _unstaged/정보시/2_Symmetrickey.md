...
- 오늘날엔 **키가 짧아** 브루트포스로 깨질 수 있어 **사용 권장되지 않음**(대신 AES/ChaCha20 등 사용).
...
- **56비트 키**: 현대 하드웨어로 **전수탐색 현실적**.
- **S-box**: IBM/NSA가 설계, **차분 분석**에 강하도록 조정된 것으로 알려짐.
- **블록 64비트**: 생일 한계로 약 **2³² 블록**(≈256GB)에서 패턴 재현 위험.

### 운용(모드)

- **ECB**: 블록 패턴 노출 → 사용 금지.
- **CBC/CFB/OFB/CTR**: 패턴 은닉 가능하나 **IV/Nonce 관리** 필수.
- 실제로는 **3DES(Triple DES)**가 오래 대체했으나, 지금은 이것도 **단종/폐기 추세**이며 **AES**로 전환이 표준.

### 왜 더는 쓰지 않나?

- 짧은 키 길이(56비트) + 저렴한 병렬 하드웨어 = **브루트포스 가능**.
- 성능/보안 측면에서 **AES-128/256, ChaCha20-Poly1305**가 우수.
