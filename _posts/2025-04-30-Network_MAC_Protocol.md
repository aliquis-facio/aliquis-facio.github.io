...
CSMA/CD는 **유선 이더넷(Ethernet)**에서 사용되는 충돌 감지형 매체 접근 제어(MAC) 방식입니다.
CSMA의 충돌 감소 기능에 충돌 감지(Collision Detection) 메커니즘을 더하여 전송 효율을 개선합니다.

동작 원리 (전송 절차)
1. 매체 감지: 전송 전 채널이 비어 있는지 확인 (Carrier Sense)
1. 전송 시도: 채널이 idle이면 데이터 전송 시작
1. 전송 중 감지: 전송 중 충돌 발생 여부를 실시간으로 감지
1. 충돌 감지: 전압 변화 등을 통해 충돌이 발생했음을 판단
1. 충돌 후 처리:
    * 즉시 전송 중단
    * Jam Signal(충돌 알림 신호)을 네트워크에 전송하여 다른 장치들에 충돌을 알림
