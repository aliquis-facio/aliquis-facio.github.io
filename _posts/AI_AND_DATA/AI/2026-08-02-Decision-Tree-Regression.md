...
1. 전체 데이터셋을 루트 노드에 배치한다.
2. 평균제곱오차(MSE)나 분산과 같은 특정 오차 지표를 최소화하는 특성과 분할 기준을 선택한다.
3. 선택한 기준에 따라 데이터를 나누고 자식 노드를 생성한다. 각 자식 노드는 해당 조건에 맞는 데이터의 부분집합을 나타낸다.
4. 정지 조건에 도달할 때까지 각 자식 노드에서 2~3단계를 반복하며 데이터를 계속 분할한다.
5. 각 리프 노드에 최종 예측값을 할당한다. 일반적으로 해당 노드에 속한 목표값의 평균을 사용한다.
