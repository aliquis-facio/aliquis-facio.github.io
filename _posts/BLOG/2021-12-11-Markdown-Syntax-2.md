...
가끔 \`(백틱)으로 감싸도 사이트에서 렌더링이 제대로 되지 않을 때가 있다. 여러 이유가 있겠지만 jekyll과 liquid를 사용하는 블로그의 경우 {% raw %} `{{...}}` {% endraw %}를 템플릿 언어인 liquid가 먼저 해석해서 오류가 발생하는 경우가 있다. 그럴 경우 {% raw %} `{% raw %}` {% endraw %} 와 {% raw %} `{% endraw %}` {% endraw %}로 앞뒤를 감싸주면 해결할 수 있다.
