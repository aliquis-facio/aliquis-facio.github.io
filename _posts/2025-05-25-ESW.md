...
EEPROM(전기적으로 지우고 다시 기록 가능한 메모리)의 원리, 인터페이스(예: I²C), 저장 공간 관리 방법과 임베디드 장치에서 펌웨어 설정·데이터 보관용으로 활용하는 예시를 설명합니다.
* [Boot Order 결정하기](https://makeutil.tistory.com/101)  
Raspberry Pi 등 SBC의 EEPROM/부트로더 설정 툴(rpi-eeprom-config)을 통해 부팅 우선순위(예: SD 카드, USB, eMMC)를 지정하는 방법과 주의점을 다룹니다.
* [펌웨어 보안을 위합 팁 5가지](https://www.boannews.com/media/view.asp?idx=50687)  
펌웨어 무결성(서명·검증), 안전한 업데이트(OTA), 접근 통제(부트 보호), 취약점 스캐닝, 롤백 전략 등 펌웨어 보안 강화 핵심 가이드라인을 제시합니다.
...
안정적인 OTA 아키텍처 설계, Delta 업데이트 기법으로 전송량 절감, 업데이트 전후 검증(해시·디지털 서명), 사용자 경험(백그라운드 설치·롤백) 최적화 방안을 다룹니다.
* [ESP-IDF와 ESP32의 간단 소개](https://dev91.tistory.com/50)  
Espressif의 공식 펌웨어 개발 프레임워크 ESP-IDF 구조, 컴포넌트(FreeRTOS, BSP), 빌드 시스템(CMake), 플래시 및 파티션 관리 개념을 개략적으로 설명합니다.
* [ESP32 IDF 펌웨어 무결성을 보호하는 방법](https://coding-by-head.tistory.com/entry/esp32-firmware-integrity)  
ESP-IDF에서 제공하는 Flash Encryption, Secure Boot 활성화 절차, 키 관리, 개발→생산 환경별 보안 설정 구분, 검증 도구 활용법 등을 정리합니다.
* [CHIPSEC Document](https://chipsec.github.io/)  
Intel 플랫폼 펌웨어·보드 레벨 보안 취약점 분석 프레임워크 CHIPSEC의 아키텍처, 설치 방법, 테스트 모듈(PCIe·SMM·ACPI 등) 사용법과 리포트 해석 가이드를 제공합니다.
...
Cisco IOS 명령어를 이용한 VLAN 생성(`vlan <id>`), 포트 Access/Trunk 모드 설정, VLAN 이름 지정, 트렁크 허용 VLAN 목록 구성 예시를 다룹니다.
* [VLAN 총정리 (개념, 명령어, 설정)](https://blog.naver.com/lunarispars/221440105402)  
VLAN 개념, 802.1Q 태깅 원리, 스위치 사이 트렁크 링크 설정, 라우터 온 어 스틱(router-on-a-stick) 구성, DHCP 스누핑·IP 소스 가드 등 보안 적용 팁을 폭넓게 정리합니다.
* [라즈베리파이에 방화벽 설정하기(feat. ufw)](https://m.blog.naver.com/simjk98/221983223546)  
ufw(Uncomplicated Firewall) 설치·기본 정책 설정, SSH/HTTP 포트 예외 허용, 규칙 활성화·비활성화, 로그 모니터링 방법을 다룹니다.
* [라즈베리파이4 보안 설정(1) - 사용자 계정 설정](https://ssoso27.tistory.com/9)  
pi 기본 계정 비활성화, 새 관리자 계정 생성·권한 설정, 강력한 비밀번호·SSH 키 인증 적용 방법을 설명합니다.
* [라즈베리파이4 보안 설정(2) - ssh 포트 변경](https://ssoso27.tistory.com/11)  
/etc/ssh/sshd_config에서 기본 22번 포트 변경, 비밀번호 인증 비활성화, SSH 데몬 재시작·접속 테스트 과정을 다룹니다.
* [라즈베리파이4 보안 설정(3) - 방화벽 설정](https://ssoso27.tistory.com/17)  
iptables 혹은 ufw로 네트워크 인터페이스별 접근 제어, 특정 IP만 허용, 기본 정책 드롭 설정 예시를 제시합니다.
* [MQTT란 무엇인가?](https://www.f5.com/ko_kr/glossary/mqtt)  
경량 메시지 큐잉·퍼블리시/서브스크라이브 구조, QoS 레벨(0/1/2), 유지 세션, Last Will & Testament, 토픽 설계 모범 사례 등을 설명합니다.
* [초간단 TLS, mTLS, E2EE](https://sang5c.tistory.com/87)  
TLS 기본 개념, 서버 인증→클라이언트 인증으로 확장한 mTLS, 종단간 암호화(E2EE) 원리와 적용 차이·장단점을 쉽게 비교합니다.
* [상호 TLS(mTLS)란 무엇일까요?](https://www.cloudflare.com/ko-kr/learning/access-management/what-is-mutual-tls/)  
클라이언트·서버 각각의 인증서 발급·관리 흐름(CA 서명), 인증서 검증 과정, 인증서 갱신·폐기 절차, 구성 예시를 다룹니다.
* [Protocol_6주차_WebRTC 구조](https://invisible-blog.tistory.com/16)  
WebRTC의 PeerConnection, ICE(Interactive Connectivity Establishment), STUN/TURN 서버 역할, 데이터·미디어 채널의 DTLS/SRTP 암호화 구조를 강의 노트 형태로 정리합니다.
* [RFC 5764: DTLS Document](https://datatracker.ietf.org/doc/html/rfc5764)  
DTLS over UDP 표준 규격 문서로, 핸드셰이크 흐름, 재전송·패킷 손실 처리, 버전 협상, 보안 매개변수(NONCE, cipher suite) 정의를 기술합니다.
* [ICE candidate](https://velog.io/@jsw4215/%EB%A6%AC%EC%95%A1%ED%8A%B8-ICE-candidate)  
ICE가 수집·교환하는 후보 주소(Type: host, srflx, relay), 우선순위(Priority) 계산, 후보 Pair 연결 테스트 과정을 설명합니다.
* [RTCIceCandidate](https://developer.mozilla.org/en-US/docs/Web/API/RTCIceCandidate)  
브라우저 WebRTC API에서 ICE 후보를 JS로 다루는 객체 속성(candidate, sdpMid, sdpMLineIndex)과 이벤트 핸들링 예시를 제공합니다.
* [SRTP, Secure Real-time Transport Protocol: 보안 실시간 전송 프로토콜](http://www.ktword.co.kr/test/view/view.php?no=6868)  
RTP에 암호화·무결성 검증을 추가한 SRTP 모드(Counter, GCM), 키 관리(SRTP KDF), Replay 보호 기법 등을 정리합니다.
...
REST 원칙(무상태, 자원 URI, HTTP 메서드 매핑, HATEOAS), RESTful 설계 모범 사례(버전 관리, 페이징, 오류 처리) 개념을 설명합니다.
* [OAuth 2.0 동작 방식의 이해](https://blog.naver.com/mds_datasecurity/222182943542)  
Grant type별 흐름(Authorization Code, Client Credentials 등), 토큰 엔드포인트·리소스 서버·인증 서버 관계, 토큰 갱신·만료 처리 과정을 다룹니다.
* [OAuth2.0 개념 및 연동](https://guide.ncloud-docs.com/docs/b2bpls-oauth2)  
Naver Cloud, AWS 등 클라우드 서비스와 OAuth2 연동 시 클라이언트 설정, 리다이렉트·scope 구성, SDK 활용 예제를 제공합니다.
* [OAuth 2.0을 사용하여 Google API에 액세스하기](https://developers.google.com/identity/protocols/oauth2?hl=ko)  
Google OAuth2 콘솔 설정, 클라이언트 라이브러리(예: google-auth), 코드 샘플(Python, JavaScript)으로 인증→API 호출 예시를 설명합니다.
* [암호 알고리즘 AES-GCM-SIV 최적화 작업 소개](https://engineering.linecorp.com/ko/blog/AES-GCM-SIV-optimization)  
AES-GCM-SIV 모드 특징(Nonce misuse resistance), LINE 엔지니어링팀의 구현 최적화(벡터화, 메모리 접근 최적화) 사례를 소개합니다.
* [RFC 8452: AES-GCM-SIV Document](https://datatracker.ietf.org/doc/html/rfc8452)  
표준 문서로 AES-GCM-SIV 모드의 프레임 구조(AAD, IV, CipherText), 사용 지침, 보안 강도 분석을 기술합니다.
* [SNORT](https://www.snort.org/)  
Snort IDS 설치·모드(IDS, IPS), 룰셋(OpenAppID·EmergingThreats), 실시간 트래픽 분석·로깅 방법을 정리합니다.
* [Snort란](https://feccle.tistory.com/52)  
Snort의 역사, 아키텍처(Packet Decoder → Preprocessors → Detection Engine → Output Plug-ins), 사용 예시(펌웨어·IoT 패턴 룰 작성) 등을 설명합니다.
* [Suricata](https://suricata.io/)  
Snort 대비 멀티스레드 처리, YAML 기반 설정, Eve JSON 출력, TLS/HTTP/SMB 등 프로토콜 디코딩 기능을 소개합니다.
* [Suricata로 트래픽 탐지](https://velog.io/@seunghyeon/Suricata%EB%A1%9C-%ED%8A%B8%EB%9E%98%ED%94%BD-%ED%83%90%EC%A7%80)  
실제 Suricata 설치 후 suricata.yaml 주요 설정(패스 타입, 규칙 경로), CLI 실행(suricata -c /etc/suricata/suricata.yaml -i eth0) 및 로그 해석 방법을 예시 중심으로 다룹니다.