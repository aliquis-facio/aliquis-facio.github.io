...
**전건이 거짓이면 후건이 무엇이든 전체 조건문은 참**
예: “네가 시험에서 100점을 받는다면, 새 휴대폰을 사주겠다” 같은 문장은 실제로 100점을 받지 않았다면 약속 위반 여부를 따질 수 없으므로 논리적으로 참으로 간주된다.
...
명제 논리에서는 문장의 실제 내용보다 **형식과 진리값의 결합 방식**이 중요하다
...
예: `P⊃Q`, `~(P∙~Q)`, `~P∨Q`가 서로 같은 의미를 가지는 표현으로 제시된다. 즉, 조건문은 다른 형태로 변환 가능하다.
