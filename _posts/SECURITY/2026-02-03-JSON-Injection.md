...
**JSON Injection**: 공격자가 JSON 문자열/스트림에 데이터를 주입하거나, 악성 JSON을 입력으로 제공해 애플리케이션의 로직, 권한, 처리 흐름을 바꾸게 만드는 취약점이다. 일반적으로 서버 측(JSON 생성/직렬화 단계)과 클라이언트 측(JSON 파싱/처리 단계)으로 구분된다.
